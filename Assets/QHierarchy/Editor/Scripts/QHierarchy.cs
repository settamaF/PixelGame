...
namespace qtools.qhierarchy
{
    ...
    public class QHierarchy
    {
        ...
        private static Dictionary<Scene, QObjectList> objectListDictionary = new Dictionary<Scene, QObjectList>();
        ...
        private static int lastSceneCount = 0;
        ...
        static void hierarchyWindowChanged()
        {
            foreach (KeyValuePair<Scene, QObjectList> objectListKeyValue in objectListDictionary)
            {
                ...
                objectList.wireframeHiddenObjects.RemoveAll(item => item == null);            
            }
        }
        ...
        static void Update()
        {
            ...
            {
                if (!inited) init();             
                if (objectListDictionary.Count == 0 || checkSceneChanged()) updateObjectListDictionary();
        ...
        public static bool checkSceneChanged()
        {
        ...
        public static QObjectList createObjectListInScene(Scene scene)
        {
            if (!objectListDictionary.ContainsKey(scene))
            {
                ...
                if (scene != gameObjectList.scene) EditorSceneManager.MoveGameObjectToScene(gameObjectList, scene);
            ...
            {
                return objectListDictionary[scene];
            }
        }
